<?php
        ...
        if ($campo != null) {
			...
			if ($query7 -> num_rows >0){
                ...
                $where = "WHERE estado='Completada' AND ID='$usuario8'";

			}
            
			else{
				
                $where = "WHERE estado='Completada' AND (";
        ...
        if ($num_rows > 0) {
            while ($row = $resultado->fetch_assoc()) {
                ...
                $output['data'] .= '<td>
                        <a class="btn btn-primary" href="Detalles_Reserva_Completadas.php?id='.$idr.'">Detalles</a>
                    </td>';
                $output['data'] .= '</tr>';
            }
        } else {
